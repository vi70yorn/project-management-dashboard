...
app.get('/api/ai/status', async (_req: Request, res: Response) => {
  try {
    const pool = getPool();
    const status = await getAIStatus(pool);
    res.json(status);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.get('/api/ai/settings', async (req: Request, res: Response) => {
  const userRole = req.headers['x-user-role'] as string | undefined;
  if (userRole && userRole !== 'admin') {
    return res.status(403).json({ error: 'Access denied: AI Settings are restricted to Admins only.' });
  }
  try {
    const pool = getPool();
    const status = await getAIStatus(pool);
...
// POST Save AI Settings (Admin only)
app.post('/api/ai/settings', async (req: Request, res: Response) => {
  const userRole = req.headers['x-user-role'] as string | undefined;
  if (userRole && userRole !== 'admin') {
    return res.status(403).json({ error: 'Access denied: Only Admins can modify AI Settings.' });
  }
  try {
    const pool = getPool();
    const { apiKey, model, enabled } = req.body;
    const updated = await saveAISettings(pool, { apiKey, model, enabled });
  ...
  try {
    ...
    if (!apiKey) {
      ...
      if (!status.configured) {
        return res.status(400).json({ error: 'No API key provided and no active key configured.' });
      }
      const { getActiveAIConfig } = await import('./aiService');
...
// POST Feature 1: Generate / Polish Acceptance Criteria
app.post('/api/ai/generate-criteria', async (req: Request, res: Response) => {
  try {
    const pool = getPool();
    const { title, projectName, existingText, mode, type } = req.body;
    if (!title && !existingText) {
      return res.status(400).json({ error: 'Title or existing text is required for AI generation.' });
    }
    const text = await generateCriteria(pool, { title, projectName, existingText, mode, type });
    res.json({ text });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
// POST Feature 3: Executive Weekly Briefing
app.post('/api/ai/weekly-briefing', async (req: Request, res: Response) => {
  try {
    const pool = getPool();
    const { weekLabel, metrics, projects, highlightTasks } = req.body;
    if (!weekLabel || !metrics) {
      return res.status(400).json({ error: 'Week label and metrics are required.' });
    }
    const result = await generateWeeklyBriefing(pool, {
      weekLabel,
      metrics,
      projects: projects || [],
...
// POST Feature 4: Risk & Workload Bottleneck Analysis
app.post('/api/ai/risk-analysis', async (req: Request, res: Response) => {
  try {
    const pool = getPool();
    const { projects, tasks, teamMembers } = req.body;
    const result = await analyzeProjectRisks(pool, {
      projects: projects || [],
      tasks: tasks || [],
...
app.post('/api/ai/chat', async (req: Request, res: Response) => {
  try {
    ...
    const reply = await chatWithCopilot(pool, { message, history, context });
    res.json({ reply });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
