...
export const TeamCapacityBarometer: React.FC<TeamCapacityBarometerProps> = ({
  ...
  const [isCollapsed, setIsCollapsed] = useState<boolean>(() => {
    try {
      return localStorage.getItem('capacity_barometer_collapsed') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const toggleCollapse = () => {
    setIsCollapsed((prev) => {
      const next = !prev;
      try {
        localStorage.setItem('capacity_barometer_collapsed', String(next));
      } catch {}
      return next;
    });
  };
  ...
  const [workloadScope, setWorkloadScope] = useState<'project' | 'all'>('project');
  ...
  // Active status check
  const isTaskActive = (task: Task, stagesList: ProjectStage[] = projectStages) => {
    const stage = stagesList.find((s) => s.name.trim().toLowerCase() === task.status.trim().toLowerCase());
    if (stage) {
      return stage.category === 'active' || stage.category === 'blocked';
    }
    return task.status !== 'Completed' && task.status !== 'Draft';
  };

  // Tasks pool based on scope
  const targetTasksPool = workloadScope === 'all' && allTasks.length > 0 ? allTasks : projectTasks;
  ...
  const memberWorkloadStats = useMemo(() => {
    ...
    return projectTeam.map((member) => {
      ...
      const activeTasks = memberTasks.filter((t) => {
        // If scope is 'all', check if task is active based on its project's stages or default
        return isTaskActive(t, projectStages);
      });
  ...
  }, [projectTeam, targetTasksPool, projectStages, currentUser?.memberId, workloadScope]);
  ...
  const teamOverview = useMemo(() => {
    ...
    const totalCapacity = projectTeam.length * DEFAULT_CAPACITY_LIMIT;
  ...
  }, [memberWorkloadStats, projectTeam.length]);

  if (projectTeam.length === 0) return null;

  return (
    <div
      id="team-capacity-barometer"
      className={`glass-panel rounded-2xl p-3 sm:p-4 transition-all duration-200 shadow-2xs mb-3.5 ${className}`}
    >
      {/* Barometer Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-2 border-b border-slate-100/90 dark:border-slate-800/80">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-2xs text-slate-500 dark:text-slate-400">
              {projectTeam.length} members • {teamOverview.totalActive} active tasks • {teamOverview.overallUtilization}% utilization (Limit: {DEFAULT_CAPACITY_LIMIT}/person)
            </p>
          </div>
        </div>
