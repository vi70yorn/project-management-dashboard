  ...
  if (isStaff && initialTask && !isOwnTask) {
    ...
    return (
      ...
      >
        <div
          id="task-modal-card"
          className="w-full max-w-xl bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]"
        >
          {/* Read-Only Header */}
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-3xs font-semibold bg-slate-200 text-slate-700 border border-slate-300">
                    <Eye className="w-2.5 h-2.5" />
                    View Detail Only ({assignedMember?.name ? `Assigned to ${assignedMember.name}` : "Team Member's Task"})
                  </span>
                </div>
                <h2 id="task-modal-title" className="text-sm font-bold text-slate-900">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {currentProject && (
                  <span className="inline-flex items-center gap-1.5 text-xs px-2.5 py-0.5 rounded-md bg-slate-100 text-slate-700 font-medium">
                    <span
                ...
                <div className="p-3 bg-white rounded-xl border border-slate-200 shadow-2xs flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-bold text-slate-900 truncate">{assignedMember.name}</p>
                  ...
                  <span className="text-2xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-medium shrink-0">
                    {assignedMember.systemRole === 'admin' ? 'Admin' : 'Staff'}
                  </span>
                </div>
              ) : (
                <div className="p-3 bg-slate-50 rounded-lg text-xs text-slate-400 italic">
                  Unassigned
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="p-3 bg-white rounded-xl border border-slate-200 shadow-2xs">
                  <span className="text-2xs text-slate-400 font-medium block">Start Date</span>
                ...
                <div className="p-3 bg-white rounded-xl border border-slate-200 shadow-2xs">
                  <span className="text-2xs text-slate-400 font-medium block">Target Due Date</span>
                  <div className="flex items-center justify-between gap-2 mt-1">
                    ...
                    {dueInfo.isToday ? (
                      <span className="text-2xs font-bold text-rose-700 bg-rose-50 border border-rose-300 px-2 py-0.5 rounded-md flex items-center gap-1 animate-pulse">
                        <AlertCircle className="w-3 h-3 text-rose-600" />
                        Due Today!
                      </span>
                    ) : dueInfo.isOverdue ? (
                      <span className="text-2xs font-semibold text-rose-700 bg-rose-50 border border-rose-200 px-2 py-0.5 rounded-md flex items-center gap-1">
          ...
          <div className="px-6 py-3.5 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
            <span className="text-2xs text-slate-500 font-medium flex items-center gap-1.5">
            ...
            <button
              ...
              type="button"
              onClick={onClose}
              className="px-5 py-2 text-xs font-semibold text-slate-700 bg-white border border-slate-300 hover:bg-slate-100 rounded-lg shadow-2xs transition-colors cursor-pointer"
            >
              Close
            </button>
          </div>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-700 mb-1">
                Status <span className="text-2xs text-slate-400">(In Progress, Pending, Blocked, Completed)</span>
              </label>
              <div className="relative">
                <select
            ...
            {isStaff ? (
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <span className="text-2xs text-slate-500">Staff Deliverable (Assigned to you)</span>
                  </div>
                </div>
                <span className="text-3xs font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
                  Your Task
                </span>
              </div>
            ) : (
              <>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-2xs text-slate-400">Assign to project member or team</span>
                  {onOpenAddMember && (
                    <button
                      type="button"
                      onClick={onOpenAddMember}
                      className="text-2xs text-blue-600 hover:underline inline-flex items-center gap-1 font-semibold cursor-pointer"
                    >
                ...
                {selectedMemberObj && (
                  <div className="flex items-center gap-2 mt-1.5 text-2xs text-slate-500">
                    ...
                    <span>&bull;</span>
                    <span className="text-slate-400">{selectedMemberObj.email}</span>
                  </div>
                )}
              </>
            )}
